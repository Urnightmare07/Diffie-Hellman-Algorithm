...
class Math:
    # Math is a module with useful math operations, it holds methods for large prime
    # numbers generation, primality test, and fast exponentiation.
    ...
    def fast_exponentiation(base: int, exponent: int, prime: int):
        # Calculate the exponentiation of integers (base ^ exponent) % prime in logarithmic time: O(lg(exponent)).
    ...
    def miller_rabin_test(number: int, tests: int = 128):
        ...
        if number == 3 or number == 2:
            return True  # random.randrange(2, number - 1) throws an exception when number is either 2 or 3.
        if number <= 1 or number % 2 == 0:
            if(number != 2):
                return False  # immediately discards if number is even or less than one.
    ...
    def generate_safe_prime_number(bits_size: int = 256):
        # Generates a safe prime number. A prime number p is safe if (p - 1) / 2 is a prime.
        ...
        while not Math.miller_rabin_test((prime - 1) // 2):  # tests if (p - 1) / 2 is prime.
            prime = Math.generate_prime_number(bits_size)  # if not, generates another prime number and retries.
        return prime
    ...
    def __generate_generator_and_prime(id: int, bits_size: int, result_queue: queue):
        ...
        factor = (safe_prime - 1) // 2  # derives a prime from safe prime.
        ...
        while exponentation == 1:  # Some guy in Stack Overflow taught this test to me.
